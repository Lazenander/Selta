...
Unless a later section fixes different bytes, every JSON metadata record
generated by the arbiter is retained as its exact JCS bytes followed by one
`0x0a` byte. The typed identity excludes that presentation newline because it
hashes JCS; `bytes_sha256` includes it. Input artifacts, requests, and model
responses retain their actual admitted bytes and are never reformatted to
imitate this generated-record convention.
...
self-bounded and may be a repository-root file such as `Cargo.lock`; naming it
does not widen discovery. Every path found transitively from those exact files
outside the three discovered inventories must be beneath at least one artifact
root. Equal roots, a root nested beneath another listed root, `.` or `..`,
symbolic links, and a file path used as a root fail admission. Derived closure
...
ASCII-lexically ordered by set-root-relative `path`; equal bytes at distinct
paths remain distinct entries. The set root is supplied by the enclosing
operation and is not serialized into the record. Each entry must resolve
beneath that root to one regular file. Symlinks, traversal, missing files,
non-regular files, case-insensitive path collisions, and byte mismatches fail
...
`assess` case in the kit index. Requests are input artifacts: their exact UTF-8
bytes are JCS with no presentation LF. Complete stdout bytes contain one
schema-valid response and no second value; diagnostics are retained only as
stderr. Every returned outcome execution carries the request implementation
digest. The transcript set contains only the runtime-evidence record, the
ledger, and one request, response, and stderr artifact per run.
