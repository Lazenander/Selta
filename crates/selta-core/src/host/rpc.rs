...
use super::{Determinism, Envelope, ExtensionDecl, ExtensionHost, HostCall, Needs};
...
pub async fn initialize_over_peer(
    ...
    let init = peer
        .call("initialize", params, INITIALIZE_TIMEOUT_MS)
        .await?;
    let manifest: proto::InitializeResult =
        serde_json::from_value(init).map_err(|e| format!("bad initialize result: {e}"))?;
    manifest
        .extensions
        .into_iter()
...
fn parse_schema(name: &str, value: Option<Value>, what: &str) -> Result<Option<Node>, String> {
    match value {
        None | Some(Value::Null) => Ok(None),
        Some(v) => Node::from_value(v)
            .map(Some)
            .map_err(|e| format!("extension '{name}': bad {what}: {e}")),
    }
}
