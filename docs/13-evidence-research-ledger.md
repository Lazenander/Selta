...
| [Darari et al., *Completeness Statements about RDF Data Sources* (2013)](https://doi.org/10.1007/978-3-642-41335-3_5) | Completeness can be stated and reasoned about relative to described data-source queries. | A completeness claim binds a mechanical channel/query scope; completeness of raw acquisition cannot silently lift to a vague semantic predicate. | RDF query completeness does not prove physical-world completeness or classifier recall. |
| [Imieliński and Lipski, *Incomplete Information in Relational Databases* (1984)](https://doi.org/10.1145/1634.1886) | Possible-world semantics represents incomplete data through the complete instances it may denote, distinguishing certain consequences from facts merely absent in the stored instance. | Under Selta's chosen open-world default, absence in a trace is not refutation unless a scoped completeness rule licenses that inference. | The database model does not establish acquisition-channel completeness or semantic truth by itself. |
| [Bareinboim and Pearl, *Controlling Selection Bias in Causal Inference* (2012)](https://proceedings.mlr.press/v22/bareinboim12.html) | Recoverability from selected data depends on structural knowledge about the selection mechanism. | Population conclusions must name their target frame and selection assumptions; preferential exclusion is not repaired by more inference over the selected graph. | The paper's graphical criteria concern causal/statistical estimands, not every semantic assessment. |
...
| [Howard et al., *Time-Uniform, Nonparametric, Nonasymptotic Confidence Sequences* (2021)](https://doi.org/10.1214/20-AOS1991) | Under stated stochastic assumptions, confidence sequences remain valid uniformly over time. | A result that advertises an error guarantee declares whether it is fixed-horizon, group-sequential, anytime-valid, or uncalibrated and binds the information history and statistic revision. | Model calls are not automatically martingale increments or IID samples; the mathematical guarantee requires its process assumptions. |
...
| [RFC 9162, *Certificate Transparency Version 2.0* (2021)](https://www.rfc-editor.org/rfc/rfc9162.html) | Signed append-only Merkle logs support inclusion and consistency auditing for submitted certificates. | An acquisition channel may use append-only commitments to detect post-commit deletion, inconsistent extension, or failure to integrate a promised submission and to bind recorded order. | A transparency log proves properties of submissions to that log; it cannot detect an event never submitted or reveal secret attempts outside its channel. |
| [Crosby and Wallach, *Efficient Data Structures for Tamper-Evident Logging* (2009)](https://www.usenix.org/conference/usenixsecurity09/technical-sessions/presentation/efficient-data-structures-tamper-evident) | Auditable tree structures let an untrusted logger prove event inclusion and consistency with earlier views. | Integrity and attribution belong to an optional audit layer over the acquisition trace; their proofs may be bound as evidence. | Tamper evidence does not establish truth, omission-free acquisition, correct time, or a single global view without additional monitoring. |
